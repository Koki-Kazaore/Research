# -*- coding: utf-8 -*-
"""compareMatchingSuccessRate.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1IsZkCqEySx3iobxaJIDeh-ubL-_bNv6G

# 概要
各々のモデルを用いて自転車を割り当てた場合の割り当て成功率の推移を比較して、モデルの優位性を確認するためのプログラム
"""
...
# それぞれのdf_time_serieのmatching_success_rateカラムの推移を比較する図をプロットする
...
# PCHIP補間
pchip_rebalance_cost_random = PchipInterpolator(time, df_time_series_random['matching_success_rate'])
rebalance_cost_smooth_random = pchip_rebalance_cost_random(time_new)

pchip_rebalance_cost_neighborhood = PchipInterpolator(time, df_time_series_neighborhood['matching_success_rate'])
rebalance_cost_smooth_neighborhood = pchip_rebalance_cost_neighborhood(time_new)

pchip_rebalance_cost_withoutStackOpt = PchipInterpolator(time, df_time_series_withoutStackOpt['matching_success_rate'])
rebalance_cost_smooth_withoutStackOpt = pchip_rebalance_cost_withoutStackOpt(time_new)

pchip_rebalance_cost_optimization = PchipInterpolator(stack_time, df_time_series_optimization['matching_success_rate'])
...
plt.plot(time_new_datetime, rebalance_cost_smooth_random, label='Random', zorder=2)
plt.plot(time_new_datetime, rebalance_cost_smooth_neighborhood, label='neighborhood', zorder=1) # Add the other plots back in
...
# 軸ラベル、凡例、タイトルを設定
plt.xlabel('Time')
plt.ylabel('matching_success_rate')
plt.legend()
plt.grid(True)
plt.title('Comparison of matching success rate by model over time')
